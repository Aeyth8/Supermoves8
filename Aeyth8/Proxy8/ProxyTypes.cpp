...
namespace DXGI
{
	...
	{
		HRESULT Result = CreateDXGIFactory_P(riid, ppFactory);
		/*if (Result >= 0 && ppFactory && *ppFactory)
		{
			DirectXHook::HookSwapChain(reinterpret_cast<IDXGIFactory*>(*ppFactory));
		}

		A8CL::Global::LogA("CreateDXGIFactory", "CALLED");*/
		return Result;
	}
	...
	{
		HRESULT Result = CreateDXGIFactory1_P(riid, ppFactory);
		/*if (Result >= 0 && ppFactory && *ppFactory)
		{
			DirectXHook::HookSwapChain(reinterpret_cast<IDXGIFactory*>(*ppFactory));
		}

		A8CL::Global::LogA("CreateDXGIFactory1", "CALLED");*/
	...
	{
		HRESULT Result = CreateDXGIFactory2_P(flags, riid, ppFactory);
		/*if (Result >= 0 && ppFactory && *ppFactory)
		{
			DirectXHook::HookSwapChain(reinterpret_cast<IDXGIFactory*>(*ppFactory));
		}

		A8CL::Global::LogA("CreateDXGIFactory2", "CALLED");*/
